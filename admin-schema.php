...
function ensure_admin_schema(mysqli $conn): void
{
    ...
    $schemaQueries = [
        "ALTER TABLE users
         ADD COLUMN role ENUM('admin', 'staff', 'user') NOT NULL DEFAULT 'user'
         AFTER email" => "SHOW COLUMNS FROM users LIKE 'role'",

        "ALTER TABLE users
         ADD COLUMN phone_number VARCHAR(30) DEFAULT NULL
         AFTER role" => "SHOW COLUMNS FROM users LIKE 'phone_number'",

        "ALTER TABLE users
         ADD COLUMN profile_image VARCHAR(255) DEFAULT NULL
         AFTER phone_number" => "SHOW COLUMNS FROM users LIKE 'profile_image'",

        "ALTER TABLE users
         ADD COLUMN bio TEXT NULL
         AFTER profile_image" => "SHOW COLUMNS FROM users LIKE 'bio'",
        ...
        )" => "SHOW TABLES LIKE 'borrowings'",

        "ALTER TABLE books
         ADD COLUMN description TEXT NULL
         AFTER genre" => "SHOW COLUMNS FROM books LIKE 'description'",

        "ALTER TABLE books
         ADD COLUMN cover_image VARCHAR(255) DEFAULT NULL
         AFTER description" => "SHOW COLUMNS FROM books LIKE 'cover_image'",

        "ALTER TABLE books
         ADD COLUMN available_for_purchase TINYINT(1) NOT NULL DEFAULT 1
         AFTER stock_quantity" => "SHOW COLUMNS FROM books LIKE 'available_for_purchase'",

        "ALTER TABLE books
         ADD COLUMN available_for_borrow TINYINT(1) NOT NULL DEFAULT 1
         AFTER available_for_purchase" => "SHOW COLUMNS FROM books LIKE 'available_for_borrow'",
    ];

    foreach ($schemaQueries as $query => $checkSql) {
    ...
    ensure_database_programmability($conn);

    $bootstrapped = true;
}

function ensure_database_programmability(mysqli $conn): void
{
    $requiredViews = [
    ...
    if ($viewResult instanceof mysqli_result) {
        while ($viewRow = $viewResult->fetch_assoc()) {
            $existingViews[] = (string) $viewRow['TABLE_NAME'];
        }
    ...
    $existingFunctions = [];
    $existingProcedures = [];
    $routineResult = $conn->query(
        "SELECT ROUTINE_NAME, ROUTINE_TYPE
         FROM information_schema.ROUTINES
         WHERE ROUTINE_SCHEMA = DATABASE()"
    );

    if ($routineResult instanceof mysqli_result) {
        while ($routineRow = $routineResult->fetch_assoc()) {
            $routineName = (string) $routineRow['ROUTINE_NAME'];
            $routineType = strtoupper((string) $routineRow['ROUTINE_TYPE']);

            if ($routineType === 'FUNCTION') {
                $existingFunctions[] = $routineName;
            } elseif ($routineType === 'PROCEDURE') {
                $existingProcedures[] = $routineName;
            }
        }
    ...
    $missingViews = array_diff($requiredViews, $existingViews);
    ...
    foreach ($routineDrops as $statement) {
        $conn->query($statement);
    }
    ...
    foreach ($routineCreates as $statement) {
        $conn->query($statement);
    }
}
